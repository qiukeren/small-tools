#This is a fork from MTOP.CC
##just display their tool
...
#author: ruzuojun
#url: www.ruzuojun.com  www.yhddba.com
#bug report email: ruzuojun@yihaodian.com
#create date: 2013-10-12
#update_date: 2013-10-28
#Important statement: The script can be free to use, but the author does not take responsibility for the script, any economic 
loss or safety issues.Thanks
...
basedir=/data/mysql # if not define. default /usr/local/mysql
...
"**********************************************************************************************************************************"
	...
	if [[ "$value" =~ "^[0-9]+$" ]] ;then
        	$bindir/mysql $login_param  -e 'show processlist'|grep -v -i -E "system 
users|replication|processlist|Sleep|Command|Connect|master|Slave|information_schema"|awk '{if($6>'$value')print $1}' |while 
read line; do $bindir/mysql $login_param  -e "kill query $line" ;  echo "mysql processes id $line been killed"; done
    	else
                echo "The value is wrong, must be a number"
        fi
	;;
    ...
    #Print dead locks info; Command: #my deadlock
        $bindir/mysql $login_param -e "select * from information_schema.INNODB_TRX a, information_schema.INNODB_LOCKS b, 
information_schema.INNODB_LOCK_WAITS c, information_schema.INNODB_TRX d where a.trx_query like '%T_PRICING_RESULT%' and 
a.trx_id=b.lock_trx_id and a.trx_id=c.requesting_trx_id and c.blocking_trx_id=d.trx_id\G"
    ;;	

    'errorlog')
    #Print last 50 line error log info; Command: #my errorlog 
    	errorlog_file=`$bindir/mysql $login_param -e "show variables like 'log_error';"|grep log_error |awk -F ' ' '{print 
$2}'`
	echo 
"**********************************************************************************************************************************"
...
"**********************************************************************************************************************************"
    ...
    'mycnf')
    #Print my.cnf info; Command: #my mycnf [parameter] ;you can find my.cnf file by #mysql  --verbose --help |grep -A 1 
'Default options'
	...
	echo "killall    kill all session if time > N seconds; Command: #my killall [N second parameter]; Example: my killall 
60"
	...
	echo "mycnf      Print my.cnf info; Command: #my mycnf [parameter] ;you can find my.cnf file by #mysql  --verbose 
